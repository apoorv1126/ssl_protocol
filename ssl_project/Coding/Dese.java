class Dese                            
{
        String ciph="";
        byte cip[];
        Dese(String key,byte[] p){
            String str1="";
            int i;
            for(i=0;i<p.length;i++)
                str1=new String(str1+(char)p[i]);
            Dese dd=new Dese(key,str1);
            cip=new byte[dd.ciph.length()];
            for(i=0;i<dd.ciph.length();i++)
                cip[i]=(byte)(dd.ciph.charAt(i));
        }
        Dese(String key,String pl)
        {
                char c;
                int k[][]=new int[17][65];
                ...
                int n,n1,nn,np,i,j,d,t,t1=0,t2,t3,k1,x,y=1,i1=0,q,q1=0,km;
                int kl[]=new int[65];
                ...
                for (x=1;x<=n1;x++)
                {
                    ...
                    for(i=0;i<8;i++){
                        temp=1;s=0;
                        for(j=8;j>0;j--){                            
                            temp1=(temp*cp[x-1][inc+j]);
                            s+=(byte)temp1;
                            temp*=2;
                        }
                        inc+=8;
                        ciph=new String(ciph+(char)s);
                    }                
                }//for end
                //System.out.println("\ncipher "+ciph);
                /*for(x=0;x<n1;x++)
                    for(j=1;j<=64;j++)
                        System.out.print(" "+cp[x][j]);*/
                cip=new byte[ciph.length()];
                //System.out.println("\n Comparision "); 
                for(i=0;i<ciph.length();i++){
                    cip[i]=(byte)(ciph.charAt(i));
                    //System.out.print(" "+cip[i]+" "+ciph.charAt(i));
                }
        }
}
